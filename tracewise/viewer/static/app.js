...
async function loadTraces(searchQuery = '', methods = []) {
    const params = new URLSearchParams();
    params.append('limit', '100');
    if (searchQuery) params.append('search', searchQuery);
    methods.forEach(m => {
        params.append('method', m);
    });
    ...
    if (traces.length === 0) {
        ...
        updateFilterStats(traces.length, searchQuery, methods);
...
function updateFilterStats(count, searchQuery, methods) {
    const filterCount = document.getElementById('filter-count');
    const clearFilterBtn = document.getElementById('clear-filter');

    if (searchQuery || methods.length > 0) {
        const filters = [];
        if (searchQuery) filters.push(`"${searchQuery}"`);
        if (methods.length > 0) filters.push(methods.join(', '));
        filterCount.innerHTML = `Showing <span class="filter-active">${count}</span> traces (filtered by ${filters.join(', ')})`;
        clearFilterBtn.style.display = 'inline';
    } else {
        filterCount.textContent = `Showing all ${count} traces`;
...
function getSelectedMethods() {
    const selected = [];
    for (let option of methodFilter.options) {
...
document.getElementById('refresh').onclick = () => {
    loadTraces(filterInput.value.trim());
};
